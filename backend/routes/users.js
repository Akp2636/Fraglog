...
router.get('/:steamId', async (req, res) => {
  try {
    ...
    const avgRating = logsWithStatus
      .filter(l => l.rating)
      .reduce((sum, l, _, arr) => sum + l.rating / arr.length, 0);

    const statusCounts = logsWithStatus.reduce((acc, l) => {
...
router.get('/:steamId/library', async (req, res) => {
  try {
    ...
    const steamRes = await axios.get(url, {
      params: {
        key: STEAM_KEY, steamid: req.params.steamId,
    ...
    // Get log statuses for these games
...
router.get('/:steamId/logs', async (req, res) => {
  try {
    ...
    const query = { steamId: req.params.steamId };
...
router.patch('/me/bio', async (req, res) => {
  ...
  try {
    ...
    req.session.user = { ...req.session.user, bio: user.bio };
    res.json({ user });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
