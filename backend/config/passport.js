...
passport.use(new SteamStrategy({
  ...
  async (identifier, profile, done) => {
    try {
      ...
      console.log(`Steam login attempt — SteamID: ${steamId}`);

      let user = await User.findOne({ steamId });

      if (!user) {
        console.log(`New user — fetching Steam profile for: ${steamId}`);
        ...
        if (!playerData) {
          console.error(`No player data for SteamID: ${steamId}`);
        ...
        // Update avatar/username on every login
        const response = await axios.get(
          `https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/`,
          { params: { key: process.env.STEAM_API_KEY, steamids: steamId } }
        );
